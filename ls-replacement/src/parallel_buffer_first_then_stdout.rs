...
pub fn buffer_first_then_stdout() {
    ...
    // Printing
    let mut buf = Arc::new(String::from(""));
    let stdout_temp = Arc::make_mut(&mut buf);
    let files: Vec<Result<DirEntry, std::io::Error>> = files_intent.unwrap().collect();
    ...
    let mapped: Vec<String> = files
        .par_iter()
        .map(|wrapped_file| {
            let file = wrapped_file.as_ref().unwrap();
            ...
            line.push_str(
                print_left_aligned(humanized_size.as_str(), columns_max_width[3]).as_str(),
            );
            line.push_str(
        ...
        .collect();
    let joined_mapped = mapped.join("\n");
    stdout_temp.push_str(joined_mapped.as_str());
    println!("{}", stdout_temp);
}
